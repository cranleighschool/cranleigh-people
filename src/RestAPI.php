...
class RestAPI
{
    ...
    {
        add_action('rest_api_init', [$this, 'wpshout_register_routes']);
        add_action('rest_api_init', [$this, 'listStaff']);
        add_filter('rest_query_vars', [$this, 'my_allow_meta_query']);
    }
    ...
    {
        $valid_vars = array_merge($valid_vars, ['meta_key', 'meta_value']);
    ...
    public function personOutput(\WP_REST_Request $request)
    {
        ...
        if ($query->have_posts()) {
            ...
            // No results
            return new WP_Error('no_staff', 'No Staff Member(s) Found', ['status' => 404]);
        }
    ...
    public function wpshout_register_routes()
    {
        register_rest_field(
            ...
            'custom_meta',
            [
                'get_callback'    => [$this, 'append_custom_meta'],
