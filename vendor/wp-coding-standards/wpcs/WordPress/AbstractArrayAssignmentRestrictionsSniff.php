...
abstract class AbstractArrayAssignmentRestrictionsSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
         ...
         * $foo['bar'] = $taz;
         */
        if (\in_array($token['code'], [\T_CLOSE_SQUARE_BRACKET, \T_DOUBLE_ARROW], true)) {
            $operator = $stackPtr; // T_DOUBLE_ARROW.
            if (\T_CLOSE_SQUARE_BRACKET === $token['code']) {
                $operator = $this->phpcsFile->findNext(\T_EQUAL, ($stackPtr + 1));
            }

            $keyIdx = $this->phpcsFile->findPrevious([\T_WHITESPACE, \T_CLOSE_SQUARE_BRACKET], ($operator - 1), null, true);
            if (! is_numeric($this->tokens[$keyIdx]['content'])) {
                $key = $this->strip_quotes($this->tokens[$keyIdx]['content']);
                $valStart = $this->phpcsFile->findNext([\T_WHITESPACE], ($operator + 1), null, true);
                $valEnd = $this->phpcsFile->findNext([\T_COMMA, \T_SEMICOLON], ($valStart + 1), null, false, null, true);
                $val = $this->phpcsFile->getTokensAsString($valStart, ($valEnd - $valStart));
                $val = $this->strip_quotes($val);
                $inst[$key][] = [$val, $token['line']];
            }
        } elseif (\in_array($token['code'], [\T_CONSTANT_ENCAPSED_STRING, \T_DOUBLE_QUOTED_STRING], true)) {
            ...
            foreach ($matches[1] as $i => $_k) {
                $inst[$_k][] = [$matches[2][$i], $token['line']];
            }
        }
        ...
        foreach ($this->groups_cache as $groupName => $group) {
            ...
            $callback = (isset($group['callback']) && is_callable($group['callback'])) ? $group['callback'] : [$this, 'callback'];
