...
abstract class AbstractClassRestrictionsSniff extends AbstractFunctionRestrictionsSniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (array_diff_key($this->groups, $this->excluded_groups) === []) {
    ...
    public function is_targetted_token($stackPtr)
    {
        ...
        if (\in_array($token['code'], [\T_NEW, \T_EXTENDS, \T_IMPLEMENTS], true)) {
            if (\T_NEW === $token['code']) {
                $nameEnd = ($this->phpcsFile->findNext([\T_OPEN_PARENTHESIS, \T_WHITESPACE, \T_SEMICOLON, \T_OBJECT_OPERATOR], ($stackPtr + 2)) - 1);
            } else {
                $nameEnd = ($this->phpcsFile->findNext([\T_CLOSE_CURLY_BRACKET, \T_WHITESPACE], ($stackPtr + 2)) - 1);
            }
        ...
        if (\T_DOUBLE_COLON === $token['code']) {
            ...
            $nameStart = ($this->phpcsFile->findPrevious([\T_STRING, \T_NS_SEPARATOR, \T_NAMESPACE], ($nameEnd - 1), null, true, null, true) + 1);
        ...
        if (\in_array($classname, ['parent', 'self', 'static'], true)) {
            return false;
        }
