...
abstract class Sniff implements PHPCS_Sniff
{
     ...
     * @var string|string[]
     */
    public $custom_test_class_whitelist = [];
     ...
     * @var array
     */
    protected $unslashingSanitizingFunctions = [
        'absint'       => true,
        'boolval'      => true,
        'count'        => true,
        'doubleval'    => true,
     ...
     */
    protected $arrayWalkingFunctions = [
        'array_map' => 1,
        'map_deep'  => 2,
    ];
     ...
     */
    protected $arrayCompareFunctions = [
        'in_array'     => true,
        'array_search' => true,
        'array_keys'   => 2,
    ];
     ...
     * @var array
     */
    protected $SQLEscapingFunctions = [
        'absint'      => true,
        'esc_sql'     => true,
        'floatval'    => true,
        'intval'      => true,
     ...
     * @var array
     */
    protected $SQLAutoEscapedFunctions = [
        'count' => true,
    ];
     ...
     * @var array
     */
    protected $cacheDeleteFunctions = [
     ...
     * @return bool
     */
    protected function addMessage($message, $stackPtr, $is_error = true, $code = 'Found', $data = [], $severity = 0)
    {
     ...
     * @return bool
     */
    protected function addFixableMessage($message, $stackPtr, $is_error = true, $code = 'Found', $data = [], $severity = 0)
    {
     ...
     * @return bool
     */
    private function throwMessage($message, $stackPtr, $is_error = true, $code = 'Found', $data = [], $severity = 0, $fixable = false)
    {
        $method = 'add';
        if (true === $fixable) {
            $method .= 'Fixable';
        ...
        return \call_user_func([$this->phpcsFile, $method], $message, $stackPtr, $code, $data, $severity);
    }
     ...
     *                          in $tokens.
     *
     * @return int Position of the last pointer on that line.
     */
     ...
     *                          in $tokens.
     *
     * @return bool True if whitelisting comment was found, false otherwise.
     */
    protected function has_whitelist_comment($comment, $stackPtr)
    {
        ...
        $end_of_statement = $this->phpcsFile->findNext([\T_CLOSE_TAG, \T_SEMICOLON], $stackPtr);
     ...
     */
    protected function is_validated($stackPtr, $array_keys = [], $in_condition_only = false)
    {
        ...
        $bare_array_keys = array_map([$this, 'strip_quotes'], $array_keys);
        $targets = [
            \T_ISSET          => 'construct',
            \T_EMPTY          => 'construct',
            \T_UNSET          => 'construct',
                ...
                case 'construct':
                    ...
                    for ($i = ($issetOpener + 1); $i < $issetCloser; $i++) {
                        ...
                        if (! empty($bare_array_keys)) {
                            ...
                            $found_keys = array_map([$this, 'strip_quotes'], $found_keys);
    ...
    public function does_function_call_have_parameters($stackPtr)
    {
        ...
        if (false === \in_array($this->tokens[$stackPtr]['code'], [\T_STRING, \T_ARRAY, \T_OPEN_SHORT_ARRAY], true)) {
            return false;
        }
     ...
     *
     *               @var int 1-based index position of the parameter. {
     *                   @var int $start Stack pointer for the start of the parameter.
     *                   @var int $end   Stack pointer for the end of parameter.
     *                   @var int $raw   Trimmed raw parameter content.
     *               }
     */
    public function get_function_call_parameters($stackPtr)
    {
        ...
        $cnt = 1;
        while ($next_comma = $this->phpcsFile->findNext([\T_COMMA, $this->tokens[$closer]['code'], \T_OPEN_SHORT_ARRAY, \T_CLOSURE], ($next_comma + 1), ($closer + 1))) {
     ...
     */
    protected function get_list_variables($stackPtr, $list_open_close = [])
    {
        if (\T_LIST !== $this->tokens[$stackPtr]['code']
