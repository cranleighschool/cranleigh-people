    ...
    public function process_token($stackPtr)
    {
        ...
        if (\T_FOR === $token['code']) {
            ...
            if (\T_CLOSE_PARENTHESIS === $this->tokens[$prev]['code']) {
                ...
                $next_statement_closer = $this->phpcsFile->findEndOfStatement($stackPtr, [\T_COLON, \T_CLOSE_PARENTHESIS, \T_CLOSE_SQUARE_BRACKET]);
                if (false !== $next_statement_closer && $next_statement_closer < $closer) {
                    // Parentheses are unrelated to the ternary.
                    return;
                }

                $prev_statement_closer = $this->phpcsFile->findStartOfStatement($stackPtr, [\T_COLON, \T_OPEN_PARENTHESIS, \T_OPEN_SQUARE_BRACKET]);
