...
class CronIntervalSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        $callbackFunctionPtr = $this->phpcsFile->findNext([\T_CONSTANT_ENCAPSED_STRING, \T_DOUBLE_QUOTED_STRING, \T_CLOSURE], $callback['start'], ($callback['end'] + 1));
        ...
        for ($i = $opening; $i <= $closing; $i++) {
            if (\in_array($this->tokens[$i]['code'], [\T_CONSTANT_ENCAPSED_STRING, \T_DOUBLE_QUOTED_STRING], true)) {
                if ('interval' === $this->strip_quotes($this->tokens[$i]['content'])) {
                    ...
                    $valueEnd = $this->phpcsFile->findNext([\T_COMMA, \T_CLOSE_PARENTHESIS], ($valueStart + 1));
                    $value = '';
