...
class I18nSniff extends AbstractFunctionRestrictionsSniff
{
    ...
    public function process_token($stack_ptr)
    {
        ...
        $this->text_domain = $this->merge_custom_array($this->text_domain, [], false);
    ...
    public function process_matched_token($stack_ptr, $group_name, $matched_content)
    {
        ...
        if (\in_array($matched_content, ['translate', 'translate_with_gettext_context'], true)) {
            $this->phpcsFile->addWarning('Use of the "%s()" function is reserved for low-level API usage.', $stack_ptr, 'LowLevelTranslationFunction', [$matched_content]);
        }
        ...
        if (! empty($arguments_tokens)) {
            $this->phpcsFile->addError('Too many arguments for function "%s".', $func_open_paren_token, 'TooManyFunctionArgs', [$matched_content]);
        }
    ...
    protected function check_argument_tokens($context)
    {
        ...
        if (empty($tokens) || 0 === \count($tokens)) {
            ...
            if ('domain' !== $arg_name) {
                $this->addMessage('Missing $%s arg.', $stack_ptr, $is_error, $code, [$arg_name]);
        ...
        if (\count($tokens) > 1) {
            ...
            $this->addMessage('The $%s arg must be a single string literal, not "%s".', $stack_ptr, $is_error, $code, [$arg_name, $contents]);

            return false;
        }

        if (\in_array($arg_name, ['text', 'single', 'plural'], true)) {
        ...
        if (\T_DOUBLE_QUOTED_STRING === $tokens[0]['code'] || \T_HEREDOC === $tokens[0]['code']) {
            ...
            foreach ($interpolated_variables as $interpolated_variable) {
                ...
                $this->addMessage('The $%s arg must not contain interpolated variables. Found "$%s".', $stack_ptr, $is_error, $code, [$arg_name, $interpolated_variable]);
            }
            ...
            if ('domain' === $arg_name && ! empty($this->text_domain)) {
                ...
                if (! \in_array($stripped_content, $this->text_domain, true)) {
                    $this->addMessage(
                        ...
                        $is_error,
                        'TextDomainMismatch',
                        [implode("' or '", $this->text_domain), $content]
                    );
        ...
        $code = $this->string_to_errorcode('NonSingularStringLiteral'.ucfirst($arg_name));
        $this->addMessage('The $%s arg must be a single string literal, not "%s".', $stack_ptr, $is_error, $code, [$arg_name, $content]);
    ...
    protected function check_text($context)
    {
        ...
        if ($unordered_matches_count > 0 && $unordered_matches_count !== $all_matches_count && $all_matches_count > 1) {
            ...
            $fix = $this->addFixableMessage(
                ...
                $is_error,
                $code,
                [implode(', ', $suggestions), implode(', ', $unordered_matches)]
            );
    ...
    {
        foreach ($args as $arg) {
            if (false === \in_array($arg['arg_name'], ['text', 'single', 'plural'], true)) {
                continue;
            }
