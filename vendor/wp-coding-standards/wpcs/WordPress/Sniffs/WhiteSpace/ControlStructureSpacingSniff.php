...
class ControlStructureSpacingSniff extends Sniff
{
     ...
     * @var array
     */
    private $ignore_extra_space_after_open_paren = [
        \T_FUNCTION => true,
        \T_CLOSURE  => true,
        \T_DO       => true,
        \T_ELSE     => true,
    ...
    public function process_token($stackPtr)
    {
                ...
                if (\T_WHITESPACE !== $this->tokens[($parenthesisCloser + 1)]['code']
                    && ! ( // Do NOT flag : immediately following ) for return types declarations.
                        ...
                        && (isset($this->tokens[$parenthesisCloser]['parenthesis_owner']) === false
                            || in_array($this->tokens[$this->tokens[$parenthesisCloser]['parenthesis_owner']]['code'], [\T_FUNCTION, \T_CLOSURE], true))
                    )
        ...
        if (isset($this->tokens[$trailingContent]['scope_condition'])
            ...
            if (\in_array($this->tokens[$owner]['code'], [\T_FUNCTION, \T_CLOSURE, \T_CLASS, \T_ANON_CLASS, \T_INTERFACE, \T_TRAIT], true)) {
